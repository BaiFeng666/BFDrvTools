...
static int __cdecl  _find(const void* a, const void* b)
{
...
void ClearPEHeadersEx(unsigned char* base) {
    ...
    DWORD size = nt_header->FileHeader.SizeOfOptionalHeader;

    if (size) {
        DWORD Protect;
        VirtualProtect((void*)base, size, PAGE_EXECUTE_READWRITE, &Protect);
        RtlZeroMemory((void*)base, size);
        VirtualProtect((void*)base, size, Protect, &Protect);
    }
}

void ClearPEHeaders(HMEMORYMODULE mod)
{
    PMEMORYMODULE module = (PMEMORYMODULE)mod;
    unsigned char* codeBase = module->codeBase;
    //memset(codeBase, 0, module->headers->OptionalHeader.SizeOfHeaders);

    if (module->headers->FileHeader.SizeOfOptionalHeader) {
        DWORD Protect;
        WORD Size = module->headers->FileHeader.SizeOfOptionalHeader;
        VirtualProtect((void*)codeBase, Size, PAGE_EXECUTE_READWRITE, &Protect);
        RtlZeroMemory((void*)codeBase, Size);
        VirtualProtect((void*)codeBase, Size, Protect, &Protect);
    }
}
...
void UnlinkModule(unsigned char* Module)
{
